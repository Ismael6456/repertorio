...
st.title('Análisis de nuestro chat de WhatsApp de yarilin e ismaelin')
###################################
###################################
...
RutaChat = '/Users/ismaelin/nuevoo/chat_whatsapp.txt'
...
# Sumar (contar) los mensajes que tengan la misma fecha
date_df = df.groupby('Fecha').size().reset_index(name='Mensaje_count')

# Plotear la cantidad de mensajes respecto del tiempo
fig = px.line(date_df, x='Fecha', y='Mensaje_count', color_discrete_sequence=['salmon'], template='plotly_dark')
...
mask_path = 'resources/corazon.jpg'
if os.path.exists(mask_path):
    mask = np.array(Image.open(mask_path))
else:
    mask = None
    st.warning(f"La imagen {mask_path} no se encuentra en la carpeta especificada.")

total_palabras = ' '.join([str(mensaje).lower() for mensaje in mensajes_df['Mensaje'].values])

# Verificar si total_palabras no está vacío
if total_palabras.strip():  # Asegura que hay contenido en total_palabras
    ...
    st.warning("No se encontraron palabras para generar la nube.")
###################################
###################################